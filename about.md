---
layout: page
title: About
permalink: /about/
---

## Profile

Chenyang Ai is a PhD student at the **Institute for Computing Systems Architecture (ICSA),
School of Informatics, University of Edinburgh**, supervised by **Prof. Nigel Topham**.
His research focuses on compute substrates for LLM inference on 3D-stacked near-memory
architectures and general-purpose tensor accelerator design.
He is expected to graduate in 2028.

## Contact

- Email: [C.Ai-4@sms.ed.ac.uk](mailto:C.Ai-4@sms.ed.ac.uk)
- Location: Edinburgh, United Kingdom
- CV: <a href="{{ '/Chenyang_Ai_FlowCV_Resume_2026-04-11.pdf' | relative_url }}">Chenyang_Ai_FlowCV_Resume_2026-04-11.pdf</a>
- GitHub: [aiiiii-creator](https://github.com/aiiiii-creator)
- LinkedIn: [chenyang-ai](https://www.linkedin.com/in/chenyang-ai-22a46b1bb/)
- 知乎: [wang-zhi-66-25-35](https://www.zhihu.com/people/wang-zhi-66-25-35)

## Research Interests

- 3D-stacked near-memory processing and compute substrates for LLM inference
- Multi-precision tensor accelerator microarchitecture
- MLIR-based compilation and scheduling for heterogeneous accelerators
- LLM-driven automatic chip generation

## Education

<div class="timeline">
  <div class="timeline-item">
    <p class="timeline-date">Sep 2025 - 2028 (expected)</p>
    <h3>University of Edinburgh</h3>
    <p>PhD in Informatics, Institute for Computing Systems Architecture (ICSA), School of Informatics.</p>
    <p>Supervisor: Prof. Nigel Topham.</p>
  </div>
  <div class="timeline-item">
    <p class="timeline-date">Sep 2022 - Jun 2025</p>
    <h3>Peking University</h3>
    <p>Master's Degree in Integrated Circuit Science and Engineering, Beijing, China.</p>
  </div>
  <div class="timeline-item">
    <p class="timeline-date">Sep 2018 - Jun 2022</p>
    <h3>Chongqing University of Posts and Telecommunications</h3>
    <p>Bachelor's Degree in Microelectronics Science and Engineering, Chongqing, China.</p>
    <p>GPA 3.79/4.0, ranked 1/35.</p>
  </div>
</div>

## Awards

<div class="award-list">
  <article class="project-card">
    <p class="project-tag">2021</p>
    <h3>National First Prize, China College Integrated Circuit Competition (Digilent Track)</h3>
    <p>Project: SoC for Sign Language and Speech Collaborative Recognition Based on CNN.</p>
  </article>
  <article class="project-card">
    <p class="project-tag">2021</p>
    <h3>Third Ranking, FudanMicroCup Digital Track</h3>
    <p>National Student Electronic Design Competition. Project: Speech Recognition Accelerator Based on LSTM.</p>
  </article>
  <article class="project-card">
    <p class="project-tag">2020</p>
    <h3>First Prize, Chongqing Provincial Level</h3>
    <p>National College Student Mathematical Modeling Competition.</p>
  </article>
</div>
