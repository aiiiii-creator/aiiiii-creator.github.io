...
On the energy axis: moving data costs far more than computing on it. On a 45 nm process, reading a 32-bit word from DRAM costs about 640 pJ, against roughly 10 pJ from an on-chip cache and 0.4 pJ for an FP32 add. The DRAM figure is Han et al.'s, and all of these are old numbers; modern LPDDR5 and HBM are several times cheaper per bit. What survives is the ratio, not the absolute values. A DRAM access costs more than 1000× the computation. Where an operator spends most of its energy is decided by its AI.
...
The most basic layout choice is row-major versus column-major, corresponding to GEMM's NN / NT / TN / TT variants. What NPUs commonly do is pre-pack the matrices further into a block format. Take Ascend's FractalNZ: the weight matrix is cut into 16×16 blocks, stored row-wise inside each block, with the blocks themselves walked column-major. That is what the name says: capital N across the blocks, lowercase z inside each one. FractalNZ is the form that sits in DDR and L1. L0B, the buffer holding the Cube's right-hand operand, holds a different member of the same family, FRACTAL_ZN, with the blocks walked the other way and transposed inside, and MTE1 re-packs one into the other on the way down from L1. Both are fractal, so when the Cube unit (a 16×16×16 engine, 4096 FP16 MACs per cycle) reads 16×16 values from L0B each cycle, it lands on exactly one contiguous physical block, and the address generator never has to stride.
...
Consider a 16-lane MAC array that consumes the 16 channel values at one (h, w) position per cycle. With NHWC data, the 16 channels of pixel (h, w) are 16 contiguous elements; the MAC reads a contiguous run from one base address and has everything in one cycle. With NCHW, those 16 channels sit at 16 addresses spaced H×W elements apart: either 16 serial reads, cutting throughput to 1/16, or spreading across the SRAM's 16 banks. With plain low-order interleaving a stride of H×W words collapses onto 16/gcd(H·W, 16) banks: whenever H·W is a multiple of 16, as at 56×56 or 28×28, every access lands in the same bank and serializes 16 ways; at 14×14 it degrades to a 4-way conflict; at 7×7 the odd stride already spreads across all 16. The collapse is a property of the address mapping, not a law. Padding the plane pitch to an odd number of words, or XOR-swizzling the bank index, breaks it for the price of a little capacity or one more address-generation stage.
...
2. **Hardware granularity**: the tile's size along the MAC array's dimensions must be a multiple of the array edge. On a 16×16×16 Cube, the M, N, and K dimensions of a tile should all be multiples of 16.
...
At the L1 level take (Bm, Bn, Bk) = (256, 256, 256); the three tiles use about 384 KB, leaving room for a double buffer. At L0 the tile shrinks to a base block sized against the 64 KB L0A and L0B, on the order of 128×256 FP16, or half that when the buffers are ping-ponged. The Cube then consumes it 16×16×16 at a time. The whole GEMM decomposes into (M/Bm) × (N/Bn) × (K/Bk) outer L1-tile iterations, each containing (Bm/16) × (Bn/16) × (Bk/16) Cube invocations.
...
Landing this tiling on an NPU has extra requirements. Four D-wide blocks are live at once: Q at Bq × D, K and V at Bk × D each, and the O accumulator at Bq × D. On top of them sits the Bq × Bk score tile, which at Bq = Bk = D = 128 is as big as the Q block and is usually kept in fp32. They need not all sit in the same buffer: on a typical NPU Q, K and V live in L1 while the scores accumulate in the matrix unit's output buffer and softmax runs in the vector core's working memory. Every one of them is scratchpad you allocate by hand, on top of double buffering and the softmax's running statistics (per-row max and sum). Ascend gives softmax its own hardware: matrix work runs on the AI Cube cores, softmax and rescaling on the separate AI Vector cores, and the two overlap. What it does not give you is a direct path between them. On 910A the cube and vector units inside one core had a direct channel; from 910B onward AIC and AIV are separate cores that hand blocks to each other through L2 and global memory. The pipelining is real, but you pay a cross-core handoff per block, and the design problem is sizing the block so that handoff stays hidden behind the Cube.
...
There are two implementation styles. One is analytic solving on a cost model: list all the constraint equations, take compute × bandwidth utilization as the objective, and solve by integer programming or exhaustive search. That works on regular operators (GEMM, Conv), where the constraints have a closed form. The other is profile-driven autotuning: candidate tile sizes are timed on real hardware and iterated toward the optimum. TVM's auto-scheduler, Triton's `@triton.autotune`, and Huawei CANN's AOE tuner all belong here, though how much search each does differs sharply: Ansor runs evolutionary search over a learned cost model that every measured batch retrains, while `@triton.autotune` times a config list you wrote by hand, once per new key shape, and caches the winner.
...
On a GPU, most wrong tile choices only cost performance: registers spill to local memory, a tile too big for L1 just pulls more traffic from L2 and HBM, and the run still finishes. Shared memory is the exception. Nothing backstops it, so a tile that overflows the per-block limit is rejected by ptxas or fails at launch. That exception is the point: on an NPU the on-chip buffers are software-managed all the way down, so what is one special case on the GPU is the rule at every level. On an NPU, a tile exceeding L1 capacity fails compilation outright, and a tile violating the MAC divisibility constraint means the Cube won't start. Tiling, like layout, is the textbook case of something going from optimization to precondition.
...
NPUs usually have several independent DMA paths, each dedicated to a different leg of the route. On Ascend these are the MTE (Memory Transfer Engine) pipes, split by direction: MTE2 brings data from off-chip memory into the on-chip buffers, MTE1 stages it from L1 down into the L0 operand buffers, and MTE3 writes vector results back out to memory. The cube's own output leg is not an MTE. FixPipe drains the L0C accumulator to L1 or GM, converting dtype and format on the way. The engines are physically independent and can work simultaneously.
...
The price is that the compiler has to generate a separate instruction stream for each engine and make sure the cross-engine dependencies are expressed correctly through synchronization primitives. In Ascend's ISA, MTE instructions and Cube instructions are issued to separate instruction queues, so the dependency between them has to be stated explicitly. `PipeBarrier<pipe>` only orders a queue against itself. Crossing from MTE1 to Cube means either a matched `SetFlag`/`WaitFlag` pair on the same hard event (`HardEvent::MTE1_M`, identical template parameter and identical event ID on both sides), or the blunt `PipeBarrier<PIPE_ALL>`, which drains every pipe and throws away the overlap you were buying. Most operator code never writes either by hand, since `TQue`'s `EnQue`/`DeQue` lowers to the same hard events, but get it wrong and the Cube array starts multiplying a half-written L0 buffer.
...
There's an optimal band for tile size: too small and DMA issue overhead dominates; too large and the pipeline is too shallow to use the hardware's concurrency. The band depends on which level the tile lands in. A tile staged into L0 or registers runs from KB to tens of KB; an L1 tile, like the 128 KB operands of the GEMM example above, runs to hundreds of KB. Either way the bounds are set jointly by DMA issue latency, bandwidth, compute density, and how many buffer copies the pipeline depth demands.
...
The scheduling algorithm is typically list scheduling or modulo scheduling. The former greedily schedules instructions one by one in dependency order — simple, not necessarily optimal. The latter targets loop structures: it overlaps successive iterations by starting a new one every II cycles, leaving a prologue, a steady-state kernel, and an epilogue, and it searches for the smallest feasible initiation interval (II), starting from the lower bound max(ResMII, RecMII) and raising it until a schedule fits. On regular operators it can approach the hardware's concurrency ceiling.
...
The metric is arithmetic intensity (AI), defined as the operator's total floating-point operations divided by the bytes it moves to and from the next level up, reads and writes both. As noted at the start, the H100's roofline knee is around 300; an operator with an AI below that is bandwidth-bound, and only above it can the compute units saturate. The numbers differ on an NPU; the structure is the same.
...
Full recomputation saves only the activation at the entry of each checkpointed segment, usually one layer, and on the backward pass re-runs the forward from that entry to regenerate the intermediates. The cost is doing the forward twice (once forward, once again during backward); the gain is activation memory down by one to two orders of magnitude. Selective schemes trade part of that saving back for speed: Megatron keeps most of a layer's activations and recomputes only the core attention operations (QK^T, softmax, softmax dropout, attention over V), which are memory-heavy but cheap to redo, about 2.7% extra FLOPs against the 30% to 40% runtime overhead of full recomputation.

Recomputation looks like the opposite of raising reuse, but it's the other pole of the same trade-off: when the cost of storage (moving data plus occupying capacity) exceeds the compute cost of regenerating it, discarding and recomputing is cheaper in total. In LLM training, where activations are enormous, recomputation is a first-class option in every major framework: PyTorch's `torch.utils.checkpoint`, Megatron's `--recompute-activations` for the selective form and `--recompute-granularity full` for whole layers. None of them is on by default. You opt in, and large-model configs usually do. The recomputation that really is on by default sits a layer lower: torch.compile's min-cut partitioner re-derives cheap pointwise ops in the backward pass instead of saving them, and FlashAttention recomputes the attention matrix inside its backward kernel.
...
The first is graph-level operator fusion. The compiler recognizes fusible patterns in the operator graph (element-wise chains, GEMM + epilogue, reduction + scale), merges several operators into a fused op, and the back end generates a single kernel for it. TVM's FuseOps and Huawei CANN's fusion engine are of this kind: FuseOps partitions a Relay dataflow DAG with a post-dominator tree, and CANN does graph fusion plus UB fusion during graph compilation. MLIR only half belongs here. `linalg-fuse-elementwise-ops` does merge ops at this level, composing an all-parallel `linalg.generic` producer into its consumer, but a producer that carries a reduction loop, a GEMM ahead of its epilogue or a reduction ahead of an element-wise op, is out of its scope. Those go through tile-and-fuse instead, which is the loop-level category below.
...
The second is loop-level schedule transformation. When generating the concrete kernel, the compiler reorders (interchange), merges (fuse), and blocks (tile — sharing the underlying mechanism with section 2) the operator's loop nest to produce the instruction sequence with the best stationarity. These systems do not share one theory. Halide, and the TVM schedule that descends from it, expose a scheduling language and infer bounds by interval analysis rather than through a polyhedral representation. MLIR's affine dialect borrows polyhedral techniques for dependence analysis, but its own rationale names MLIR's ability to represent arbitrary control flow as a key distinction from polyhedral implementations like Polly. From the outside the transformations look identical. What differs is how much each system can prove about them.
...
| Tiling | tile exceeds L1 and compilation fails outright | usually just slower (spills, extra L2/HBM traffic), except a tile over the shared-memory limit, which fails to compile or launch |
